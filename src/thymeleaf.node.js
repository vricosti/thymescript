...
class ThymeleafJs {
  ...
  evaluate(expression, context) {
    ...
    const variableRegex = /(^|[^\w.'"])([\w_][\w\d_]*)($|[^\w'"])/g;
    const functionCallRegex = /[\w_]+\.[\w_]+\([^)]*\)/g;
    ...
    while ((expressionMatch = expressionRegex.exec(expr)) !== null) {
      ...
      // Remove function calls to avoid capturing function names as variables
      expression = expression.replace(functionCallRegex, '');

      let variableMatch;
      while ((variableMatch = variableRegex.exec(expression)) !== null) {
        // Only add the match to variables if it is not immediately followed by a dot
        if (variableMatch.index + variableMatch[0].length === expression.length || expression[variableMatch.index + variableMatch[0].length] !== '.') {
          variables.add(variableMatch[2]);
        }
      }
    }
    ...
    // Once again very very simple code to know if we should include lists/arrays utilites
    ...
    let funcCode = functionsToInject.length ? this.injectFunctions(functionsToInject) : '';

    //console.log('contextKeys: ', contextKeys);
    const jsExpr = `\n${funcCode}\n ${undefinedValuesExpr} return (${expr});`;
